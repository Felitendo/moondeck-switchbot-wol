...
declare -A MSB_EN=(
    ...
    [host_done]="Done, the trigger is listening.\n\nPaste this one line into the installer on your Steam Deck:\n\n%s\n\nIt carries the port, the secret and this machine's current address. The address is only a fallback: the Deck first tries whatever MoonDeck itself uses to reach this host, so a new DHCP lease does not break anything.\n\nThe secret alone stays readable in /etc/moondeck-login-agent/secret.\n\nRemove all of it again with:\ncurl -fsSL https://raw.githubusercontent.com/Felitendo/moondeck-switchbot-wol/main/host/install-host.sh | bash -s -- --uninstall"
    ...
    [host_keyring_warning]="One more thing: this machine has a password protected secret store (%s).\n\nIt is normally unlocked with your password while you log in. With an automatic login nobody types one, so it stays locked and the first application that wants a stored secret pops up a password dialog.\n\nThe only way around that is a secret store without a password: seahorse for gnome-keyring, the KWallet settings for KWallet. The stored secrets then sit unencrypted on disk, which is the trade you make for logging in without typing anything."
    ...
    [host_token]="Paste this one line into the installer on your Steam Deck:\n\n%s"
...
declare -A MSB_DE=(
    ...
    [host_done]="Fertig, der Trigger lauscht.\n\nDiese eine Zeile im Installer auf dem Steam Deck einfügen:\n\n%s\n\nDarin stecken Port, Geheimnis und die aktuelle Adresse dieses Rechners. Die Adresse ist nur die Rückfallebene: das Deck probiert zuerst die, über die MoonDeck den Host selbst erreicht, ein neuer DHCP-Lease macht also nichts kaputt.\n\nDas Geheimnis allein steht weiterhin in /etc/moondeck-login-agent/secret.\n\nAlles wieder entfernen mit:\ncurl -fsSL https://raw.githubusercontent.com/Felitendo/moondeck-switchbot-wol/main/host/install-host.sh | bash -s -- --uninstall"
    ...
    [host_keyring_warning]="Noch eine Sache: auf diesem Rechner liegt ein passwortgeschützter Schlüsselspeicher (%s).\n\nDer wird sonst beim Anmelden mit deinem Passwort aufgeschlossen. Beim automatischen Login tippt niemand eins, also bleibt er zu, und die erste Anwendung, die ein gespeichertes Geheimnis braucht, öffnet ein Passwortfenster.\n\nDagegen hilft nur ein Schlüsselspeicher ohne Passwort: bei gnome-keyring über Seahorse, bei KWallet über die KWallet-Einstellungen. Die Geheimnisse liegen dann unverschlüsselt auf der Platte - das ist der Preis dafür, sich ohne Tippen anzumelden."
    ...
    [host_token]="Diese eine Zeile im Installer auf dem Steam Deck einfügen:\n\n%s"
