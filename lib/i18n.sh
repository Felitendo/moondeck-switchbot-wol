...
declare -A MSB_EN=(
    ...
    [host_ntfy_ask]="Send a push notification through ntfy once the host is up and its stream port answers?"
    ...
    [host_ntfy_ready]="%s finished booting, is logged in and its stream port answers."
...
declare -A MSB_DE=(
    ...
    [host_ntfy_ask]="Eine Push-Benachrichtigung über ntfy schicken, sobald der Rechner oben ist und der Stream-Port antwortet?"
    [host_ntfy_topic]="ntfy-Topic (wer es kennt, kann deine Benachrichtigungen mitlesen):"
    [host_ntfy_server]="ntfy-Server:"
    [host_ntfy_title]="PC ist bereit"
    [host_ntfy_ready]="%s ist hochgefahren, angemeldet und der Stream-Port antwortet."
