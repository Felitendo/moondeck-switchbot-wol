...
declare -A MSB_EN=(
    ...
    [host_done]="Done, the trigger is listening.\n\nPaste this one line into the installer on your Steam Deck:\n\n%s\n\nIt carries everything the Deck needs: the port, the trigger secret, this machine's current address and, if you said yes, the SwitchBot setup too. The address is only a fallback - the Deck first tries whatever MoonDeck itself uses to reach this host, so a new DHCP lease does not break anything.\n\nAnything in it that is stored here at all stays in /etc/moondeck-login-agent/.\n\nRemove all of it again with:\ncurl -fsSL https://raw.githubusercontent.com/Felitendo/moondeck-switchbot-wol/main/host/install-host.sh | bash -s -- --uninstall"
...
declare -A MSB_DE=(
    ...
    [host_done]="Fertig, der Trigger lauscht.\n\nDiese eine Zeile im Installer auf dem Steam Deck einfügen:\n\n%s\n\nDarin steckt alles, was das Deck braucht: Port, Trigger-Geheimnis, die aktuelle Adresse dieses Rechners und, falls du zugestimmt hast, auch die SwitchBot-Einrichtung. Die Adresse ist nur die Rückfallebene - das Deck probiert zuerst die, über die MoonDeck den Host selbst erreicht, ein neuer DHCP-Lease macht also nichts kaputt.\n\nWas davon hier überhaupt gespeichert wird, liegt in /etc/moondeck-login-agent/.\n\nAlles wieder entfernen mit:\ncurl -fsSL https://raw.githubusercontent.com/Felitendo/moondeck-switchbot-wol/main/host/install-host.sh | bash -s -- --uninstall"
